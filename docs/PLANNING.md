...
- `admin_daily_stats` 배치 집계 잡을 어느 프로세스가 돌릴지(app APScheduler vs celery worker) — 이 코드베이스는 celery 워커가 앱 컨텍스트 없이 떠서 DB 작업이 실패하는 알려진 이슈가 있음([[project-facereview]] 메모리 참고) → **APScheduler 쪽에 붙이는 것을 권장**.
- 회원/댓글 수가 지금보다 훨씬 커지면(수십만~수백만) 현재 "가벼움"으로 분류한 항목도 배치 전환이 필요해질 수 있음 — 데이터량 재확인 후 판단.
